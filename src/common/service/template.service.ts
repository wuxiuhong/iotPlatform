...
function TemplateService(data: any, _self: any = null) {
    ...
    /**
     * 数据来源的可控性
     * @param {string} deviceid 查询的数据来源的设备id
     * @param {string} keyname 数据获取微服务中配置的标识，对应一个查询的指令
     * @param {any} parstr 查询指令中使用到的参数，此参数需要经过url编码
     * @param callback 回调函数
     */
    function biSearch(deviceid: string, keyname: string, parstr: any, version: string, callback: any) {
        this.ctx.$.ajax({
            url: `http://10.149.18.37:888/api/datacapture/${version}/${deviceid}/${keyname}`,
            data: {
                parm: JSON.stringify(parstr)
            },
            type: 'get', dataType: 'json', async: true, xhrFields: {
