<template>
    <div class="dashboard-container" :style="dashboard.styles"
        ...
        <Breadcrumb :aliases="[dashboard.dashboardTitle ]"></Breadcrumb>
        <!--视图 start-->
...
<script lang="ts">
    ...
    import { renderFn, subscribeEdgeClient, WebsocketService } from '../../common';
    ...
    export default class DashboardDetail extends Vue {
        ...
        created() {
            ...
            this.resize = _.debounce(() => {
                this.dashboard.components.forEach((item: any) => {
                    this.$refs && this.$refs[item.pref] && this.resizeWidget(item, this.$refs[item.pref][0].$refs);
                });
            }, 1000);
        }
        ...
        mounted() {
            ...
            // 初始化报表数据
            DashboardsApi.getDashboard(this.$route.params).then((ret: any) => {
                // 处理报表配置json值
                if (ret.code === 0) {
                    this.dashboard = {
                        ...ret.data,
                        ...JSON.parse(ret.data.dashboardConfiguration)
                    };
                    // 处理报表数据格式
                    this.dashboard = formatDashboard(this.dashboard);
                    // 处理组件数据格式
        ...
        formatTemplate(template: any, index: number) {
            ...
            return DashboardsApi.getWidget({
                id: template.templateId,
                version: template.templateVersion
            }).then((t: any) => {
                template.template = {
                    ...t.data,
                    ... JSON.parse(t.data.widgetDescriptor)
                };
                template = renderFn(template, index, this);
                if (template.template.components && template.template.components.length) {
                    axios.all(template.template.components.map((child: any, cindex: number) => {
